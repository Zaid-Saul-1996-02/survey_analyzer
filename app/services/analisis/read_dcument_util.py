...
def get_data_excel(url_excel:str) -> list[list]:
    ...
    print(f"\n{'='*20}")
    print("Iniciando get_data_excel...")
    print("="*20)
    ...
    index_pos = df_excel\
        .index[df_excel.iloc[:, 1].notna()]\
        .tolist()
    
    print(f"\n{'='*20}")
    print("Index pos obtenidos correctamente")
    print("="*20)
    ...
    for ind in range(len(index_pos) - 1):
        ...
        nombre_capitulo = df_excel.iloc[ini_cap, 1]\
                                .replace('\n', ' ')\
                                .replace('nan', '')\
                                .rstrip()
        ...
        index_pos_sub = df_cap[df_cap.iloc[:, 2].notna()]\
                                                .index\
                                                .tolist()
        ...
        for ind2 in range(len(index_pos_sub) - 1):
            ...
            df_sub_cap = df_cap.loc[ini_sub_cap:fin_sub_cap - 1]\
                            .replace('\n', ' ', regex=True)\
                            .fillna('')
